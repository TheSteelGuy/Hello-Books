    ...
    try:
       if request.method == 'POST':
          ...
          if response == "succsefully logged in":
              ...
              return make_response(jsonify(
                  {'message':"Incorrect credentials for admin"}
              )), 401
    except:
        return make_response(jsonify(
            {'message':'Please Ensure, data passed in form of:{}'.format(
                {
                    'key1':'value1',
                    'key2':'value2'
                    })}
...
@admin.route('/api/v1/auth/reset',methods=['POST'])
def reset_default_password():
    ...
    if request.method == 'POST':
       ...
       if not username or new_username or password or new_pwd or email:
           return make_response(jsonify(
